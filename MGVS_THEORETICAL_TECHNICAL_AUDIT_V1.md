...
현 상태는 **논문 핵심 알고리즘과 초기 falsification gate를 통과한 research prototype**이다.
이 문서 작성 후 수행한 18-regime frozen G9-3 검증에서는 end-to-end superiority 가설이
실패했다. 최종 판정은 `G9_FROZEN_FALSIFICATION_REPORT_2026-07-18.md`를 우선한다.
...
이는 development evidence다. 이후 untouched multi-regime confirmatory run에서
raw/smoothed single-level work ratio는 geometric `1.101x`로 `2x` gate에 실패했고,
개선 core regime도 `7/12`에 그쳤다. 반면 raw/smoothed MLMC work ratio `1.926x`와
모든 exactness gate는 통과했다.

초기 aggregate 하한 계산은 공통 시드를 쓴 regime-by-seed 120개 cell을 독립으로 취급한
pseudoreplication 오류가 있었다. 저장 결과와 실행 코드는 fixed 12-regime suite를 10개
validation seed로 cluster하여 one-sided 95% 하한을 `1.025x`로 수정했다. regime
heterogeneity 민감도 하한은 `0.990x`이므로 population-wide superiority는 주장하지 않는다.
또한 `1.101x`는 online variance-times-cost 비율이다. 공통 CEM calibration과 MGVS 전용
direction search를 포함한 정확한 training-inclusive 비율은 측정되지 않았으며, 추가 MGVS
비용이 음수가 아니므로 이를 포함해도 실패한 `2x` gate를 회복할 수는 없다.
...
Frozen G9-3 실패로 현재 one-coordinate MGVS를 end-to-end 주모델로 제출해서는 안 된다.
...
- `results/g9_mgvs_frozen_v3_2026-07-18.json`
- `G9_FROZEN_FALSIFICATION_REPORT_2026-07-18.md`

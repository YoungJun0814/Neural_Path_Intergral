...
## 8. V7 Linux 재현이 추가로 확인한 것

Windows 결과와 겹치지 않는 새 seed를 사용해 Linux Docker 환경에서 같은
64-cluster 연구를 반복했다. estimator source는 Windows와 동일한 clean commit
`80a5bc9`이고, protocol ID와 accuracy bootstrap seed 외의 과학 설정은 바꾸지
않았다.

| Linux raw/DCS 지표 | 기하평균 비율 | one-sided 95% 하한 |
|---|---:|---:|
| 동일 경로 probe 분산 | 3.3727 | 3.3244 |
| production 실행 분산 | 3.5172 | 3.4499 |
| final sampling work | 3.3721 | 3.3021 |
| training 포함 전체 work | 1.8909 | 1.8613 |
| final wall time | 2.7634 | 2.7042 |

모든 mechanism gate, 72개 accuracy claim, 독립 record 감사, 독립 종합 감사가
통과했다. Windows와 Linux의 필수 효과 차이는 합성 표준오차 단위로 최대
1.706이었고 사전 기준 3.0 이하였다. 네 개 seed 집합 사이의 모든 교집합도
0이었다.

단, 이것은 같은 노트북에서 Windows와 Linux/WSL2 Docker를 비교한
**소프트웨어 환경 재현**이다. 독립 물리 하드웨어나 외부 연구팀 재현이라고
부르면 안 된다. final wall-time 차이는 8.798 표준오차로 컸고, OS scheduler와
container overhead의 영향을 받기 때문에 사전에 진단값으로만 정했다. 반면
분산과 operation-work 효과는 안정적으로 재현됐다.
...
Linux 재현에서도 72개 주장이 모두 통과했다. 최소 목표달성률 하한은 0.8310,
최대 RMSE 상한/허용치 비는 0.7530이었다. DCS의 개별 miss는 2/1,152, raw는
4/1,152였고 모두 결과에 남겨 두었다.
...
- clean estimator commit을 read-only mount한 Linux 재현;
- Windows/Linux probe/fixed 네 seed 집합의 모든 쌍 교집합 검사;
- canonical artifact와 reproduction artifact의 SHA-256 연결 검사;
...
1. task-tuned CEM, adaptive IS, 가능하면 flow/transport 계열과 동일한
   training-inclusive 비교;
2. barrier/mesh 이론 강화 또는 finite-grid 범위를 명확히 제한한 정리;
3. 외부 proof와 코드 review;
4. 독립 물리 하드웨어 또는 외부 연구팀 재현;
...
- `experiments/g11_v7_freeze_linux_reproduction.py`: 결과 전 Linux freeze 생성;
- `experiments/g11_v7_hardware_reproduction.py`: Windows/Linux 재현성 감사;
...
- `docs/audits/G11_V7_MECHANISM_QUALIFICATION_V1_DECISION_2026-07-24.md`:
  자격검증 최종 판정;
- `docs/audits/G11_V7_MECHANISM_CONFIRMATION_V1_DECISION_2026-07-24.md`:
  Windows confirmation 최종 판정;
- `docs/audits/G11_V7_LINUX_REPRODUCTION_V1_DECISION_2026-07-24.md`:
  Linux 재현 및 Windows/Linux 비교 판정.

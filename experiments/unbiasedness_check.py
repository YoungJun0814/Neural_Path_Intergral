...
def main(N: int = 50_000, u: float = 0.5, T: float = 0.5, K: float = 100.0,
         linear_slope: float | None = None) -> dict:
    ...
    S_is, _v, log_w, _bh, _ = sim.simulate_controlled(
        S0=100.0, v0=0.04, T=T, dt=1 / 252.0, num_paths=N,
        control_fn=ctrl, apply_v_drift_correction=True,
    )
    ...
    S_bad, _, log_w_bad, _, _ = sim.simulate_controlled(
        S0=100.0, v0=0.04, T=T, dt=1 / 252.0, num_paths=N,
        control_fn=ctrl, apply_v_drift_correction=False,
    )
    weighted_bad = torch.clamp(S_bad[:, -1] - K, min=0.0) * torch.exp(log_w_bad)
    mean_bad = float(weighted_bad.mean())

    report = {
        "N_mc": N, "N_is": N, "T": T, "K": K, "control": ctrl_label,
        "mean_mc": mean_mc, "se_mc": se_mc,
        "mean_is_corrected": mean_is, "se_is": se_is,
        "mean_is_uncorrected": mean_bad,
        "diff_corrected": diff, "3sigma_band": band,
